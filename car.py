...
class CarAgent(Agent):
    ...
    async def setup(self):
        print("Agente carro começou")

        class CarBehaviour(CyclicBehaviour):
            async def run(self):
                #print(f"carro a funcionar: {self.agent.jid}")
                ...
                self.agent.environment.createCar(self.agent.jid, self.agent.posicao_carro_X, self.agent.posicao_carro_Y, self.agent.sentido_carro, self.agent.status)
                await asyncio.sleep(0.1)
        self.add_behaviour(CarBehaviour())